...
LfLeg::LfLeg() : Leg() {
    ...
    {
        cout << "error in LfLeg ctor: " << Error.what() << endl;
    }
    //A_leg_R.printMatrix();
}

Result LfLeg::setInverseKinematics(double px, double py, double pz) {
...
Result LfLeg::legForwardBackWard(double xInit, double zInit, double zFin, int forward) {
    return Leg::legForwardBackWard(-xInit, zInit, zFin, forward);
}

Result LfLeg::legRightLeft(double zInit, double xInit, double xFin, int side) {
    return Leg::legRightLeft(zInit, -xInit, -xFin, side);
}

Result LfLeg::legTurn(double xInit, double xFin, double zInit, double zFin) {
    return Leg::legTurn(-xInit, -xFin, zInit, zFin);
}

void LfLeg::print(ostream& co) {
